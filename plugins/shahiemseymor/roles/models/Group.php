...
class Group extends Model
{
    ...
    public static function hasRole($role, $user = null)
    {
    	...
    	$account = new Account;
        $userRoles = json_decode(User::find($user == null ? $account->user()->id : $user)->frontend_groups);
    ...
    public static function can($permissions)
    {
        ...
        if(Auth::check())
        {
        	$roles = json_decode(User::find($account->user()->id)->frontend_groups);
    ...
    public function newUserAddToDefaultGroup()
    {
        User::created(function($user) {
            ...
            $user->frontend_groups()->sync($defaultGroups);
        });
    }
